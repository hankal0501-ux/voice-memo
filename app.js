...
document.getElementById('fileInput').addEventListener('change', async (e) => {
  ...
  try {
    if (file.name.endsWith('.zip')) {
      ...
      // 사진 먼저 복원 (구버전 photos/, 신버전 사진/ 모두 지원)
      const photoMap = {};
      let storageFull = false;
      for (const [path, zipFile] of Object.entries(zip.files)) {
        if (zipFile.dir) continue;
        if (!(path.startsWith('photos/') || path.startsWith('사진/'))) continue;
      ...
      if (storageFull) {
        showToast('⚠️ 저장 공간 부족: 일부 사진만 복원됨', 'error');
      }
...
async function doSaveToDevice(customName) {
  ...
  // ① 메인 문서 HTML (PDF 인쇄 가능)
  const escHtml = s => String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
  pageData[currentPage] = getTableRows();
  const memoPgNums = Object.keys(pageData).map(Number).sort((a,b)=>a-b);

  // 모든 페이지 사진 ID → base64
  const imgMap = {};
  for (const pg of memoPgNums) {
      ...
      for (const cell of row) {
        if (typeof cell === 'string' && cell.startsWith('__IMG__')) {
          ...
          if (!imgMap[id]) {
            ...
            if (!src) {
              try { const d = await idbGet('images', id); if (d?.src) src = d.src; } catch {}
            }
  ...
  document.querySelectorAll('.cell-photo-icon').forEach(el => {
    ...
    if (id && !imgMap[id] && el.dataset.imgSrc) imgMap[id] = el.dataset.imgSrc;
  });

  let memoBodyHtml = '';
  memoPgNums.forEach(pg => {
    const rows = pageData[pg] || [];
    if (memoPgNums.length > 1) memoBodyHtml += `<h3>${pg}페이지${pageDongNames[pg] ? ' - ' + escHtml(pageDongNames[pg]) : ''}</h3>`;
    else if (pageDongNames[pg]) memoBodyHtml += `<h3>${escHtml(pageDongNames[pg])}</h3>`;
    memoBodyHtml += `<table><thead><tr><th style="width:8%">층</th><th style="width:15%">위치</th><th>점검내용</th><th style="width:15%">점검결과</th></tr></thead><tbody>`;
    rows.forEach(row => {
      memoBodyHtml += `<tr>`;
      row.forEach(cell => {
        if (typeof cell === 'string' && cell.startsWith('__IMG__')) {
          const id = cell.replace('__IMG__','');
          const src = imgMap[id];
          memoBodyHtml += `<td>${src ? `<img src="${src}" style="max-width:80px;max-height:80px;">` : `[사진 ${id}]`}</td>`;
        } else {
          memoBodyHtml += `<td>${escHtml(cell)}</td>`;
        }
      });
      memoBodyHtml += `</tr>`;
    });
    memoBodyHtml += `</tbody></table>`;
  });

  const memoHtml = `<!DOCTYPE html><html lang="ko"><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escHtml(biz)}</title>
<style>
...
<button class="btn-print" onclick="window.print()">📄 PDF 저장</button>
  ...
  zip.file(`noimg_${biz}_${dateStr}.html`, memoHtmlNoImg);
